...
def draw_stuff(task, viewer):
    ...
    # Draw feet positions
    lfoot_pos = task._client.get_lfoot_body_pos()
    rfoot_pos = task._client.get_rfoot_body_pos()
    viewer.add_marker(pos=lfoot_pos, size=np.ones(3)*0.05, rgba=[0.5, 0.5, 0.5, 1], type=sphere, label="")
    viewer.add_marker(pos=rfoot_pos, size=np.ones(3)*0.05, rgba=[0.5, 0.5, 0.5, 1], type=sphere, label="")
...
def run(env, policy, args):
    ...
    while (ts < end_ts):
        ...
        if env.__class__.__name__ == 'JvrcWalkEnv':
