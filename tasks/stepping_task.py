...
####
from .construction_tasks import ConstructionEnvironment, ConstructionTypes
...
class WalkModes(Enum):
    ...
    LATERAL = auto()
####
    CONSTRUCTION = auto()  # New mode for construction environment
...
####
        self.construction_env = ConstructionEnvironment()
...
####
        self.mode = np.random.choice(
            [ WalkModes.BACKWARD, WalkModes.FORWARD, WalkModes.CONSTRUCTION],
            p=[ 0.0, 0.01, 0.99])
...
####
        if self.mode == WalkModes.CONSTRUCTION:
             # Randomly choose construction type
            construction_type = np.random.choice(list(ConstructionTypes))
            sequence = self.construction_env.generate_construction_sequence(construction_type)

        else:
            if self.mode == WalkModes.BACKWARD:
                d['step_size'] = -0.1
                ...
                h = np.clip((self.iteration_count-3000)/8000, 0, 1)*0.1 # was *0.1
    ...
    ## CREATE TERRAIN USING GEOMS
        nboxes = 80  # Changed from 20 to 50
        boxes = ["box"+repr(i+1).zfill(2) for i in range(nboxes)]
        sequence_extended = [np.array([0, 0, -1, 0]) for i in range(nboxes)]
        sequence_extended[:len(self.sequence)] = self.sequence

        if self.mode == WalkModes.CONSTRUCTION:
            # Using construction environment visualization
            self.construction_env.adjust_terrain_visualization(self._client, boxes, self.sequence)
        else:
            # Original visualization for other modes
            for box, step in zip(boxes, sequence_extended):
